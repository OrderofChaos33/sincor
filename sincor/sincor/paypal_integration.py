...
class PayPalIntegration:
    def __init__(self):
        # Accept both common PayPal environment variable names
        self.client_id = os.getenv('PAYPAL_REST_API_ID') or os.getenv('PAYPAL_CLIENT_ID')
        self.client_secret = os.getenv('PAYPAL_REST_API_SECRET') or os.getenv('PAYPAL_CLIENT_SECRET')
        
        # PayPal API configuration - default to sandbox for safety
        ...
        if not self.client_id or not self.client_secret:
            raise ValueError("PayPal credentials not found. Ensure PAYPAL_CLIENT_ID and PAYPAL_CLIENT_SECRET (or PAYPAL_REST_API_ID and PAYPAL_REST_API_SECRET) are set in Railway")
        
        # Log configuration (without exposing secrets)
        env_mode = "sandbox" if self.sandbox_mode else "live"
        print(f"PayPal integration initialized: {env_mode} mode, credentials present: {bool(self.client_id and self.client_secret)}")
    ...
    async def get_access_token(self) -> str:
        ...
        try:
            ...
            if not self.client_id or not self.client_secret:
                raise Exception("PayPal credentials missing")
...
# Global payment processor instance (initialized only when credentials are available)
...
def get_payment_processor():
    ...
    if payment_processor is None:
        try:
            ...
            print(f"Warning: PayPal not configured - {e}")
            payment_processor = None
