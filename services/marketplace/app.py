...
async def purchase_item(purchase: PurchaseRequest):
    ...
    try:
        ...
        if payment_provider == "paypal":
            # PayPal integration (placeholder - implement PayPal SDK)
            payment_intent = {
                "id": f"paypal_{purchase.item_id}_{datetime.now().timestamp()}",
        ...
        purchase_id = await app.state.db.fetchval("""
            INSERT INTO purchases (tenant_id, customer_email, item_id, amount_cents, payment_provider, payment_ref, status)
            VALUES ($1, $2, $3, $4, $5, $6, 'pending')
            RETURNING id
        """, purchase.tenant_id, purchase.customer_email, purchase.item_id, 
            item['price_cents'], payment_provider, payment_intent["id"] if isinstance(payment_intent, dict) else payment_intent.id)
